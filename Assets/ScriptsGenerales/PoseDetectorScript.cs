...
public class PoseDetectorScript : MonoBehaviour 
{
 ...
 /*   void Start()
    {
        poseModel = GameObject.Find("modelo").GetComponent<PoseModelHelper>();
        avatarModel= GameObject.Find("jugador").GetComponent<PoseModelHelper>();
    }
*/
	void Update () 
	{
		   ...
		   avatarModel != null && avatarCtrl && kinectManager.IsUserTracked(avatarCtrl.playerId))
		{
            ...
            if (fMatchPercent == 1)
                GameManager.percent = 0;
            else
                GameManager.percent = fMatchPercent;
